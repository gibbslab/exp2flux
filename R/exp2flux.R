...
exp2flux <-
  function(model,
    ...
    download.file(
      "rest.kegg.jp/list/organism",
      paste0(keggDownload, "organism.txt"),
      quiet = TRUE,
      method = "curl"
    )
    keggOrganism <-
    ...
    if (!is.null(organism) & !is.null(typeID)) {
      organismPathways <-
        downloadPathways(organism = organism, typeID = typeID)
    } else {
      organismPathways <- NULL
    }
    
    # Expression Values for each GPR
    expressionValues <- gprExpression(
      gpr = model@gpr,
    ...
    if (scale == TRUE) {
      expressionValues <-
        round((expressionValues / max(expressionValues)) * 1000)
    }
    ...
    # Exchange reactions
    model@lowbnd[model@react_id %in% findExchReact(model)@react_id] <-
      lb[model@react_id %in% findExchReact(model)@react_id]
    model@uppbnd[model@react_id %in% findExchReact(model)@react_id] <-
      ub[model@react_id %in% findExchReact(model)@react_id]
    
    # Change model name
    model@mod_id <- outputID
    model@mod_name <- paste0(outputID," model generated in exp2flux")
    # Return
    return(model)
  }